...
public class GetMerakiClient {
  ...
  public GetMerakiDTO execute(final String startDate, final int perPage) {
    WebClient webClient = WebClient.builder().baseUrl(merakiConfig.getBaseUrl()).build();
    String url = String.format("/networks/%s/clients?perPage=%d&t0=%s",
        merakiConfig.getNetworkId(), perPage, startDate);
